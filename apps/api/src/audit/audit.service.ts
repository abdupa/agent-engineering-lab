...
function buildGoal(rubric: string): string {
  return [
    ...
    'on, and a findingId. Read the lines. If they are not what your claim is about, call',
    'report-finding again with the corrected line and replaces set to that findingId. That',
    'corrects the finding in place. Reporting it again without replaces leaves both the',
    'wrong citation and the right one in the report, which is worse than either alone.',
    'A claim attached to the wrong line is worse than no finding, because a reviewer',
    'cannot tell it is wrong without rechecking the file themselves.',
    '',
