...
Status: **Released 2026-09-18.** See [RELEASE.md](RELEASE.md) and [REVIEW.md](REVIEW.md).
...
| v0.7-004 | Metrics over (record, key): precision, recall, citation accuracy, severity agreement, duplicate-claim rate, and cost per **located defect** rather than per finding — more findings is not better, and a rate that rewards volume would reward the duplicates run 12 produced. Pure functions, no I/O. | Hand-computed expected values on synthetic records. Denominators reported beside every rate, so 1/1 never prints as 100%.                                                                                   |
...
| v0.7-006 | Baseline and regression gate in the suite.                                                                                                                                                                                                                                                             | A committed baseline file; the gate fails on **any** difference, not only a drop, because a floor lets the scorer become more generous undetected; proved by making a breaking change and watching it fail. |
